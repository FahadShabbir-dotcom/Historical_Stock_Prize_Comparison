...
upper_limit = st.number_input('Enter the Upper limit :', min_value=0, max_value=100000, value=10)
lower_limit = st.number_input('Enter the Lower limit :', min_value=0, max_value=100000, value=100)
...
try:
    ...
    filtered_df = merged_df[(merged_df['HIGH_history'] >= upper_limit) & (merged_df['LOW_today'] <= lower_limit)]
