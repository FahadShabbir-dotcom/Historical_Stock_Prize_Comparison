...
try:
    ...
    # Plotting the HIGH_history and LOW_today data using Plotly Bar Chart
    st.header('HIGH History vs LOW Today Bar Plot')
    
    # Prepare data for plotting
    fig = px.bar(filtered_df, x='Name', y=['CLOSE_history', 'CLOSE_today'],
                 title='HIGH History vs LOW Today',
                 labels={'value': 'Value', 'Name': 'Names'},
                 barmode='group',  # Bars for HIGH_history and LOW_today side by side
                 text_auto=True)   # Show bar values automatically

    # Customize the layout for beauty
    fig.update_layout(
        title={'x': 0.5},
        xaxis_title="Names",
        yaxis_title="Value",
        legend_title="Types",
        template="plotly_dark",  # Dark theme, you can change this if needed
